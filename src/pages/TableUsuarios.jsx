...
const TableUsuarios = () => {
  ...
  const [isModalOpenCrearSesion, setIsModalOpenCrearSesion] = useState(false) // Nuevo estado para el modal de crear sesión
  ...
  const handleCrearSesion = (profesor) => {
    setUserId(profesor.data.userId); // Guardar el userId
    setIsModalOpenCrearSesion(true); // Abrir el modal de crear sesión
  }
  ...
  return (
    <>
            ...
            <div className="overflow-x-auto rounded-t-lg">
              <table className="min-w-full divide-y-2 divide-gray-200 bg-white text-sm">
                <thead className="ltr:text-left rtl:text-right">
                  <tr>
                    <th className="whitespace-nowrap pr-1 pl-4 py-2 font-medium text-gray-900">Nombre</th>
                    <th className="whitespace-nowrap px-2 py-2 font-medium text-gray-900">Correo</th>
                    <th className="whitespace-nowrap px-2 py-2 font-medium text-gray-900">ID</th>
                ...
                <tbody className="divide-y divide-gray-200">
                  {currentItems.map((profesor) => (
                    <tr key={profesor.id}>
                      <td className="whitespace-nowrap px-2 py-2 text-gray-700 text-center">{profesor.data.displayName}</td>
                      <td className="whitespace-nowrap px-2 py-2 text-gray-700 text-center">{profesor.data.email}</td>
                      <td className="whitespace-nowrap px-2 py-2 text-gray-700 text-center">{profesor.data.userId}</td>
                      <td className="td-des px-2 py-2 h-full column gap-2 text-gray-700 text-center cursor-pointer">
                        <div className="contenedor-btn flex justify-center h-full pt-2 gap-2">
                          <button
                            onClick={() => handleCrearSesion(profesor)} // Llamar a la función para crear sesión
                            type="button"
      ...
      {isModalOpen && (
        ...
        {isModalOpenCrearSesion && ( // Modal para crear sesión
          <div className="cont-modal " onClick={() => setIsModalOpenCrearSesion(false)}>
            <div className="modal w-[450px] md:w-[600px] lg:w-[600px]" onClick={handleModalStop}>
              <button className="bg-transparent hover:bg-white text-white font-semibold hover:text-gray-600 py-2 px-6 border border-white hover:border-transparent rounded mb-2" onClick={() => setIsModalOpenCrearSesion(false)}>Cerrar</button>
              <ModalCrearSesionFotos userId={userId} /> {/* Pasar el userId al modal */}
            </div>
          </div>
        )}
        </div>
